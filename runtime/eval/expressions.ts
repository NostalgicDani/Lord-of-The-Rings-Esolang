import { BinaryExpr, Identifier, AssignmentExpr, ObjectLiteral, CallExpr } from "../../frontend/ast.ts";
import Environment from "../environment.ts";
import { evaluate } from "../interpreter.ts";
import { FunctionValue } from "../values.ts";
import { RuntimeVal, NumberVal, MK_NULL, ObjectVal, NativeFnValue } from "../values.ts";
...
export function eval_assignment(node: AssignmentExpr, env: Environment): RuntimeVal {
  if(node.assigne.kind !== "Identifier"){
    throw `Cannot assign with LHS inside assignment expression ${JSON.stringify(node.assigne)}`;
  }
...
export function eval_call_expr(expr: CallExpr, env: Environment): RuntimeVal {
  ...
  if (fn.type == "function"){
    ...
    for(let i = 0; i < func.parameters.length; i++){
      //check the bounds here, verify arity of function
      const varname = func.parameters[i];
      scope.declareVar(varname, args[i], false);
    }

    let result: RuntimeVal = MK_NULL();
    for (const stmt of func.body){ //eval function body line by line
